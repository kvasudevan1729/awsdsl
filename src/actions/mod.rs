...
pub(crate) struct AwsDeployError {
    pub(crate) err_type: AwsErrorType,
    pub(crate) message: String,
}

impl AwsDeployError {
    pub(crate) fn new(err_type: AwsErrorType, msg: String) -> Self {
        AwsDeployError {
            err_type: err_type,
            message: msg,
        }
...
pub(crate) async fn apply_aws(aws_sym: &AwsSym) -> Result<(), AwsDeployError> {
    ...
    for ec2 in &aws_sym.ec2s {
        ...
        match ec2_inst {
            ...
            Err(e) => {
                errors.push(AwsDeployError {
                    err_type: AwsErrorType::EC2Deploy,
                    message: format!("err: {}", e),
                });
            }
        }
    }
    if !errors.is_empty() {
        ...
        for err in &errors {
            s = format!("{}\n{:?}:: {}", s, err.err_type, err.message);
        }
        return Err(AwsDeployError {
            err_type: AwsErrorType::EC2Deploy,
            message: s,
        });
    }
